...
export interface ResultPdfSection {
  heading: string;
  columns: string[];
  rows: (string | number)[][];
}
...
export interface ResultsPdfPayload {
  ...
  title: string;
  subtitle: string;
  meta: string; // e.g. "Exam: Mid Term Examination | Session: 2026 - 2027"
...
/**
 * Builds a branded results PDF and triggers the browser download
 * immediately (doc.save).
 */
export const exportResultsPdf = (payload: ResultsPdfPayload): void => {
  ...
  doc.text(payload.meta, pageWidth - 40, 28, { align: "right" });
    ...
    autoTable(doc, {
      ...
      body:
        section.rows.length > 0
          ...
          : [
              {
                content: "No records found for the selected scope.",
