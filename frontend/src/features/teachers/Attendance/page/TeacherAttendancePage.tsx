...
const TeacherAttendancePage = () => {
  ...
  // Filter values â€” everything comes from the admin's
  ...
  // effective value derived during render â€” this avoids setters in
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(
        `Attendance saved for ${loadedLabel} on ${date} â€” ${present} present, ${absent} absent`
      );
    } catch (error) {
      toast.error(
        error instanceof Error
    ...
    <>
      ...
      <div className="mt-6">
          {students === null ? (
            ...
            <Card
              title={`${loadedLabel} Â· ${date}`}
              subtitle={`${students.length} students â€” ${presentCount} present, ${absentCount} absent`}
